...
navigator.id.watch({
  ...
  onlogin: function(assertion) {
    if (typeof me !== "undefined" && me !== null) {
      return $('#userinfo').fadeOut('normal', function() {
        return wait_id(assertion);
      });
    } else {
      $("form#browserid input").val(assertion);
      return $("form#browserid").submit();
    }
  },
  onlogout: function() {
    if (window.logged_in_user) {
      $('#userinfo').fadeOut();
      return window.location = "/logout?return=" + (encodeURIComponent(window.location.href));
    } else {
      return $('#userinfo').fadeIn();
    }
  }
});

wait_id = function(assertion) {
  var xhr;
  if (me.id.length !== 40) {
    return setTimeout(wait_id, 137);
  }
  xhr = $.post("/auth/link", {
    assertion: assertion,
    id: me.id,
  ...
  return xhr.error(function(data) {
    $('#userinfo').fadeIn();
    return logAnnotation(data.responseText);
  });
};
...
var addAnnotation, addImportant, banButton, boxxyAnnotation, chatAnnotation, congressionalAnnotation, createAlert, guessAnnotation, logAnnotation, notifyTrolls, userSpan, verbAnnotation,
...
chatAnnotation = function(_arg) {
  ...
  if (done) {
    ...
    if (text === '' || (text.slice(0, 1) === '@' && text.indexOf(me.id) === -1 && user !== me.id)) {
      ...
      if (user in room.users && text.length > 70 || dirty_regex.exec(" " + text + " ")) {
        banButton(user, line);
      }
    }
  } else {
...
render_categories = function() {
  ...
  for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
    ...
    if (percentage > 0) {
      _results.push($(item).find('.name').css('font-weight', 'bold'));
    } else {
...
$('.dist-picker .increase').live('click', function(e) {
  ...
  obj[$(item).data('value')]++;
  return me.set_distribution(obj);
});

$('.dist-picker .decrease').live('click', function(e) {
  var cat, item, obj, s, val, _ref;
  if (!room.distribution) {
    return;
  }
  item = $(this).parents('.category-item');
...
renderPartial = function() {
  ...
  if (!room.question) {
    ...
    if ((info != null ? info.generated_time : void 0) !== (typeof room !== "undefined" && room !== null ? room.generated_time : void 0) || (typeof room !== "undefined" && room !== null ? room.question : void 0) !== (info != null ? info.question : void 0)) {
      changeQuestion();
    }
  }
  updateTextPosition();
  ...
  for (id in _ref2) {
    ...
    if (user.tribunal) {
      ...
      if (user.elect) {
        ...
        $('.elect-' + user.id).slideUp('normal', function() {
          return $(this).remove();
        });
      }
    }
...
toggle_bookmark = function(info, state) {
  var b, bookmarks;
  me.bookmark({
    id: info.qid,
    value: state
...
create_report_form = function(info) {
  ...
  _ref = ["Wrong category", "Wrong details", "Broken question"];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    option = _ref[_i];
...
create_bundle = function(info) {
  var breadcrumb, bundle, field, readout, star, well;
  bundle = $('<div>').addClass('bundle').addClass("qid-" + info.qid);
...
QuizPlayer = (function() {
  ...
  QuizPlayer.prototype.touch = function(no_add_time) {
    ...
    this.last_action = current_time;
    return this.room.check_timeout();
  };
  ...
  QuizPlayer.prototype.nominate = function() {
    ...
    if (!this.elect) {
      ...
      this.__elect_timeout = setTimeout(function() {
        _this.verb('got romneyed', true);
        _this.elect = null;
  ...
  QuizPlayer.prototype.create_tribunal = function() {
    ...
    if (!this.tribunal) {
      ...
      this.__tribunal_timeout = setTimeout(function() {
  ...
  QuizPlayer.prototype.deserialize = function(obj) {
    var attr, blacklist, val, _results;
    blacklist = ['tribunal', 'elect'];
    _results = [];
    for (attr in obj) {
      val = obj[attr];
      if (attr[0] !== '_' && __indexOf.call(blacklist, attr) < 0) {
...
QuizRoom = (function() {
  ...
  QuizRoom.prototype.check_timeout = function() {
    if (this.attempt) {
      if (this.serverTime() > this.attempt.realTime + this.attempt.duration) {
        this.clear_timeout();
  ...
  QuizRoom.prototype.new_question = function() {
    var _this = this;
    this.generating_question = this.serverTime();
    return this.get_question(function(question) {
      ...
      _this.timing = (function() {
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          word = _ref1[_i];
          _results.push((+syllables(word) || 0) + 1);
        }
        return _results;
      }).call(_this);
  ...
  QuizRoom.prototype.set_speed = function(rate) {
    ...
    this.end_time = this.begin_time + new_duration + this.answer_duration;
    if (isNaN(this.begin_time)) {
      this.begin_time = 0;
    }
  ...
  QuizRoom.prototype.next = function() {
    if (this.generating_question && this.serverTime() - this.generating_question < 1000) {
      delete this.generating_question;
      return;
    }
    if (this.time() >= this.end_time && !this.attempt) {
      this.unfreeze();
  ...
  QuizRoom.prototype.buzz = function(user, fn) {
    ...
    } else if (!this.attempt && this.time() <= this.end_time) {
      if (fn) {
...
offline_startup = function() {
  initialize_offline(function() {
    ...
    return load_bookmarked_questions();
  });
  return setTimeout(function() {
